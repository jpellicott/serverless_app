...
Make sure that the FastAPI code is working correctly by running it locally using `uvicorn`:
```bash
...
1. Download & install Mangum (https://mangum.io/)
2. Download & install Python-lambda (https://github.com/nficano/python-lambda)
3. Mangum changes to `service.py`
4. Create a local test event (`event.json`)
5. Create the `config.yaml`
6. Create the Dockerfile from `public.ecr.aws/lambda/python:3.8`

#### Build Docker image & test the container locally

Build the Docker image now. 
```bash
# Docker build command
docker build -t news_classifier_lambda .
```

Testing this Docker image locally is a few steps. 
```bash
# Spin up the container using the image that was just created
docker run -p 9000:8080 news_classifier_lambda:latest

# Optional: `exec` into the container to make sure that files are copied correctly and
# packages are installed correctly
docker exec -it <container_id> bash # get container_id from `docker ps`
```

Make requests to the FastAPI app running inside this container:

cURL request: https://gist.github.com/nihit/63c86e8ceb71181410d41aad2bcb8d7d 


#### Push Docker image and Update Lambda

[1] Login to the ECR repository
aws ecr get-login-password --region us-west-2 | docker login --username AWS --password-stdin 120271050898.dkr.ecr.us-west-2.amazonaws.com
...
```bash
docker tag news_classifier_lambda:latest 120271050898.dkr.ecr.us-west-2.amazonaws.com/news-classifier:latest

docker push 120271050898.dkr.ecr.us-west-2.amazonaws.com/news-classifier:latest
```

3. Update lambda function: 


Update the AWS Lambda to start using the latest pushed image:
```bash
aws lambda update-function-code --function-name <lambda function name> --image-uri 120271050898.dkr.ecr.us-west-2.amazonaws.com/news-classifier:latest
```

```bash
 aws lambda create-function --function-name news_classifier --package-type Image --code ImageUri=120271050898.dkr.ecr.us-west-2.amazonaws.com/news-classifier:latest --role arn:aws:iam::120271050898:role/lambda_role
 ```
